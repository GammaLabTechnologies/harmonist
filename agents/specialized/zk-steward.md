...
is_background: false
model: reasoning
tags: [knowledge-management, zk-proofs, ai, growth, strategy]
domains: [all]
version: 1.0.0
...
Companion outputs: execution plan (`YYYYMMDD_01_[Book_Title]_Execution_Plan.md`), atomic/method notes, index note for the topic, workflow-audit report. See **deep-learning** in the companion skill set (see note at the bottom of this agent).
...
ZK Steward’s workflow references these capabilities. They are not part of this repo; use your own tools or the ecosystem that contributed this agent:
...
*Companion skill definitions (Cursor/Claude Code compatible) — small YAML/markdown skills like `deep-learning`, `atomic-note`, `random-walk`, `strategic-advisor`, `workflow-audit` — can be dropped under `.cursor/skills/` in the host project. This pack ships only the agent definition; the companion skills are optional and not bundled.*
