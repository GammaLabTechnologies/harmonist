...
# Keep in sync with scripts/convert.sh and scripts/lint_agents.py (NON_AGENT_DIRS).
AGENT_DIRS=(
  academic
  design
  engineering
...
tool_label() {
  case "$1" in
    ...
    openclaw)    printf "%-14s  %s" "OpenClaw"     "(~/.openclaw/harmonist)" ;;
...
interactive_select() {
  ...
  while true; do
    ...
    printf "\n"
    box_top
    box_row "${C_BOLD}  Agent Catalog -- Tool Installer${C_RESET}"
    box_bot
    printf "\n"
...
install_gemini_cli() {
  ...
  local dest="${HOME}/.gemini/extensions/harmonist"
  local count=0
...
install_openclaw() {
  ...
  local dest="${HOME}/.openclaw/harmonist"
  local count=0
