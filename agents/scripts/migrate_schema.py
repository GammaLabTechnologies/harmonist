...
ALLOWED_DOMAINS = {
    ...
    "gov-tech",
    "education",
    # Authorized offensive security (recon / exploitation / red team).
    # Deliberately NOT `all`: these personas must only surface in projects
    # that explicitly declare a pentest engagement, instead of being
    # visible to every project's candidate pool gated only by prose.
    "pentest",
}
...
FIXED_DOMAINS: dict[str, list[str]] = {
    ...
    # Authorized offensive-security personas: pinned to the `pentest`
    # domain so they never enter a generic project's candidate pool.
    # security-vulnerability-triage stays [all] -- defensive triage is
    # useful everywhere.
    "security-exploit-developer":                          ["pentest"],
    "security-recon-mapper":                               ["pentest"],
    "security-red-team-operator":                          ["pentest"],
    "security-web-app-pentester":                          ["pentest"],
    "security-pentest-report-writer":                      ["pentest"],
