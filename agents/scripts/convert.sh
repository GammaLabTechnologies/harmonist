#!/usr/bin/env bash
#
# convert.sh — Convert pack agent .md files into tool-specific formats.
#
...
#                    conversion (see scripts/extract_essentials.py). Source
#                    files untouched; integration outputs stay under token budget.
...
# Sub-folders (e.g. game-development/unity/) are walked recursively by `find`.
# Keep in sync with scripts/install.sh and scripts/lint_agents.py (NON_AGENT_DIRS).
AGENT_DIRS=(
  academic
  design
  engineering
...
# Strips surrounding single or double quotes so callers see the raw value
# (Schema v2 quotes values that start with '#', '[', etc.).
...
run_conversions() {
  local tool="$1"
  local count=0
  # If THIN_MODE=1 each agent is fed through extract_essentials.py before
  # conversion so the integration output stays under token budget.
  local thin_tmp=""
  [[ "${THIN_MODE:-0}" == "1" ]] && thin_tmp="$(mktemp -d)"
  ...
  for dir in "${AGENT_DIRS[@]}"; do
    ...
    while IFS= read -r -d '' file; do
      ...
      # Thin mode: extract essentials into a scratch file and feed THAT to the
      # per-tool converter. Leaves the source files untouched.
      local effective_file="$file"
      if [[ -n "$thin_tmp" ]]; then
        effective_file="$thin_tmp/$(basename "$file")"
        python3 "$SCRIPT_DIR/extract_essentials.py" "$file" > "$effective_file"
      fi

      case "$tool" in
        antigravity) convert_antigravity "$effective_file" ;;
        gemini-cli)  convert_gemini_cli  "$effective_file" ;;
        opencode)    convert_opencode    "$effective_file" ;;
        cursor)      convert_cursor      "$effective_file" ;;
        openclaw)    convert_openclaw    "$effective_file" ;;
        qwen)        convert_qwen        "$effective_file" ;;
        kimi)        convert_kimi        "$effective_file" ;;
        aider)       accumulate_aider    "$effective_file" ;;
        windsurf)    accumulate_windsurf "$effective_file" ;;
      esac
